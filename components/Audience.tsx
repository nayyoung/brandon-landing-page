...
const AudienceCard: React.FC<{ title: string; desc: string; icon: React.ReactNode; index: number }> = ({ title, desc, icon, index }) => (
  <div 
    className="group p-6 sm:p-8 border border-white/10 bg-charcoal-800 hover:bg-charcoal-700 transition-all duration-300 rounded-sm hover:-translate-y-2 hover:border-copper-500/50"
  >
    <div className="mb-4 sm:mb-6 p-3 sm:p-4 inline-flex items-center justify-center bg-charcoal-900 rounded-full text-copper-500 group-hover:bg-copper-500 group-hover:text-white transition-colors duration-300">
      {icon}
    </div>
    <h3 className="font-display text-lg sm:text-xl font-bold text-white mb-2 sm:mb-3">{title}</h3>
  ...
  return (
    <Section id="audience" className="bg-charcoal-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="mb-10 sm:mb-16 md:max-w-2xl">
